...
public class ExpandableBreathingButton extends View {
    ...
    /**
     * 关于文字位置居中计算，用Rect计算不准确，
     * 可能是因为返回的宽高是int值，应该用FontMetrics类来计算
     */
    @Override
        ...
        if (mBackgroundRectFLeft == mArcWidth / 2) {
            ...
            for (int i = 0; i < mItemTextArr.length; i++) {
                ...
                canvas.drawText(mItemTextArr[i], 0, mItemTextArr[i].length(),
                        mItemWidth * i + mItemWidth / 2 - itemTextWidth / 2,
                        mOuterRadius + mArcWidth / 2 - (itemFontMetrics.ascent + itemFontMetrics.descent) / 2,
                        mItemTextPaint);
            }
        }
    }
    ...
    /**
     * 关于可展开效果，其实就是点击发布时，启动一个ValueAnimator，
     * 对一个圆角矩形的左边距离不断改变
     */
