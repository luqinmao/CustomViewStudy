...
import com.lqm.customview.utils.DensityUtils;

import static com.lqm.customview.View.HorizontalExpandMenu.ButtonStyle.Right;

/**
 * user：lqm
 * desc：可展开收起的水平菜单栏
 * url:https://juejin.im/post/5a32b21a6fb9a0452b494402
 */
...
public class HorizontalExpandMenu extends RelativeLayout {
    ...
    private void init() {
        ...
        defaultWidth = DensityUtils.dp2px(mContext, 200);
        defaultHeight = DensityUtils.dp2px(mContext, 40);
        ...
        menuCornerRadius = typedArray.getDimension(R.styleable.HorizontalExpandMenu_corner_radius, DensityUtils.dp2px(mContext, 20));
     ...
     * ViewGroup会先测量子View的大小位置（onLayout），然后再进行绘制，如果子View或background为空，
     * 则不会调用draw方法绘制。当然我们可以调用setWillNotDraw(false)让ViewGroup可以在子View
     * 或background为空的情况下进行绘制，但我们会为ViewGroup设置一个默认背景
     */
